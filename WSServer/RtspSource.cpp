...
{
    public:
        fmp4databuf(uint8_t* data, int len)
        {
            buflen = len;
...
static int write_buffer(void *opaque, uint8_t *buf, int buf_size)
{
    ...
    fmp4databuf* fdata = new fmp4databuf(buf, buf_size);
    if (av_fifo_space(source->fmp4buf) < sizeof(fmp4databuf*))
        av_fifo_realloc2(source->fmp4buf, av_fifo_size(source->fmp4buf) + sizeof(fmp4databuf**) * 16);
    av_fifo_generic_write(source->fmp4buf, &fdata, sizeof(fmp4databuf**) , NULL);
    return buf_size;
}
...
RtspSource::RtspSource(std::string url, websocketpp::connection_hdl hdl)
    ...
    , fmp4buf(NULL)
{
    fmp4buf = av_fifo_alloc(sizeof(fmp4databuf**) * 16);
}
...
bool RtspSource::Open()
{
    ...
    {
        if (((In_FormatContext->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) && (In_FormatContext->streams[i]->codecpar->codec_id == AV_CODEC_ID_H264))
                || ((In_FormatContext->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO)
                    && ((In_FormatContext->streams[i]->codecpar->codec_id == AV_CODEC_ID_AAC) || (In_FormatContext->streams[i]->codecpar->codec_id == AV_CODEC_ID_AAC_LATM))))
        {
    ...
    av_dump_format(Out_FormatContext, 0, "", 1);
    ...
    boost::thread SendWSThread(&RtspSource::SendWSData,this);
    return true;
}
...
{
    Runing = false;
    while (av_fifo_size(fmp4buf) >= sizeof(fmp4databuf**))
    {
        fmp4databuf* fdata = NULL;
        av_fifo_generic_read(fmp4buf, &fdata, sizeof(fmp4databuf**), NULL);
        delete fdata;
    }
...
void RtspSource::ReadAndWrite()
{
    ...
    while (Runing)
    {
        ...
        //ret = av_write_frame(Out_FormatContext, &pkt);
        av_packet_unref(&pkt);
    }
...
void RtspSource::SendWSData()
{
    while (Runing)
    {
        if (av_fifo_size(fmp4buf) >= sizeof(fmp4databuf**))
        {
            fmp4databuf* fdata = NULL;
            av_fifo_generic_read(fmp4buf, &fdata, sizeof(fmp4databuf**), NULL);
            sendBinaryData(fdata->buf,fdata->buflen);
            delete fdata;
        }
        else
        {
